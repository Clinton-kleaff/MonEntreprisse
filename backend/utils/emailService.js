...
const adminEmail = process.env.ADMIN_EMAIL || 'admin@wagradetech.ht';

// Helper: get the client URL dynamically, fallback to production domain
const getClientUrl = () => {
  return process.env.CLIENT_URL || 'https://wagradetech.onrender.com';
};
    ...
    <div style="max-width:600px; margin:0 auto; padding:20px;">
      <div class="card" style="background:#ffffff; border-radius:24px; box-shadow:0 10px 25px -5px rgba(0,0,0,0.05), 0 8px 10px -6px rgba(0,0,0,0.02); overflow:hidden; padding:32px 28px;">
        ...
        <p style="margin:0; font-size:12px; color:#8898aa; text-align:center;">
          &copy; ${new Date().getFullYear()} WaGradeTech. Tous droits réservés.<br>
          <a href="${clientUrl}" style="color:#d81b60; text-decoration:none;">WaGradeTech.ht</a>
        </p>
      </div>
    </div>
  </body>
...
export const sendWelcomeEmail = async (email, name) => {
  ...
  const inner = `
    <div style="text-align:center; margin-bottom:24px;">
      <h1 style="font-size:28px; font-weight:700; color:#1a2c3e; margin:0 0 8px;">Bienvenue chez WaGradeTech 🚀</h1>
  ...
  `;
  const html = getBaseTemplate(inner, 'Bienvenue chez WaGradeTech');
  return sendEmail(email, 'Bienvenue chez WaGradeTech', html);
};
...
export const sendOrderConfirmationToUser = async (orderData) => {
  ...
  const inner = `
    ...
    <p style="font-size:16px;">Merci d’avoir fait confiance à WaGradeTech. Voici le récapitulatif de votre demande :</p>
...
export const sendNewsletterConfirmation = async (email) => {
  ...
  const inner = `
    <div style="text-align:center;">
      ...
      <p style="font-size:14px;">À très bientôt,<br>L’équipe WaGradeTech</p>
